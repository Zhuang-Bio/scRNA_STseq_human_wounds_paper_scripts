...
```{r}
saveRDS(all, file = "../../deconv/inputs/bulks/bulk_sobject.rds")
```
...
saveRDS(deconvolution_crc, file=file.path(outdir, "scdc_prop_obj_full.rds"))
...
SaveH5Seurat(tmp, filename = file.path(outdir,"bulk_data_full.h5Seurat"), overwrite = T)
Convert(file.path(outdir,"bulk_data_full.h5Seurat"), dest = "h5ad", overwrite = T)

file.remove(file.path(outdir,"bulk_data_full.h5Seurat"))
```
