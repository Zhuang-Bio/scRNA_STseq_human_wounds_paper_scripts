...
# Bas_I    Bas_mig Bas_prolif      Gra_I      Spi_I   Spi_II_a   Spi_II_b    Spi_III    Spi_mig 
# 41         39         39         40         41         41         41         31         41 

# choose the cell types for plotting
alldf_test <- alldf %>% dplyr::filter(newCellType == "Bas_mig") 
alldf_test <- alldf %>% dplyr::filter(newCellType == "Spi_mig") 
...
p1.wds <- ggplot(acw, aes(Condition, NormProp, colour=Condition)) +
  ...
  #scale_y_continuous(limits = c(0,8),breaks = c(0, 2,4,6,8,10,12)) +
  theme_classic() +
...
##########################################################
# FOSL1 positive cells proportion in all integrated dataset
library(scCustomize)
alldf <- data.table::fread("allInte_CellType_Keratinocyte_cellproportion.txt")

inteData <- readRDS("../01-keratinocyte/allacuteWound_DFU_VU_inteKC_redu.rds")
genes_all <- FetchData(inteData, vars = c("Condition", "orig.ident", "Project", "newCellType",
                                          "FOSL1"), slot = "data")
genes_all_f <- genes_all %>% filter(FOSL1 > 0)

sm.tol <- table(inteData$orig.ident) %>% as.data.frame()

FOSL1 <- table(genes_all_f$orig.ident, genes_all_f$newCellType) %>% as.data.frame() %>% 
...
skin_vu <- allexp %>% dplyr::filter(Project == "VU") %>% 
  dplyr::filter(! orig.ident %in% c("NS23")) %>% # weird expression of FOSL1 in this sample
  group_by(orig.ident) %>% mutate(Prop_tot=sum(Prop)) %>% ungroup() %>% 
...
#pdf("Result_CellProportion_allInte_KC_FOSL1positive.pdf", useDingbats = FALSE, width = 4, height = 2)
p1.wds + p1.dfu + p1.vu + plot_layout(widths = c(2,1.5,1))
dev.off()
...
data.table::fwrite(exp_sig, "Result_CellProportion_allInte_KC_FOSL1positive_sig.txt", sep = "\t")

AverageExpression(inteData, features = c("FOSL1"), assays = "RNA", group.by = "Condition")
...
pdf("Result_CellProportion_allInte_KC_MKI67positive.pdf", useDingbats = FALSE, width = 4, height = 2)
p1.wds + p1.dfu + p1.vu + plot_layout(widths = c(2,1.5,1))
dev.off()
...
exp_sig <- rbind(t(ct_acute), t(ct_dfu), t(ct_vu)) %>% as.data.frame() %>% rownames_to_column(var = "Group")
data.table::fwrite(exp_sig, "Result_CellProportion_allInte_KC_MKI67positive_sig.txt", sep = "\t")
