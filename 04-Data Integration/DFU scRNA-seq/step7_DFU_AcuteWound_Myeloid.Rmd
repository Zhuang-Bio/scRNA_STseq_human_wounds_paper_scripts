...
DFU_skin <- subset(inteData, subset = CellType == "M1_Macro" | CellType == "M2_Macro")
...
DimPlot(mye_sub, group.by = "upCellTypes", cols = ct.cols)
mye_sub <- DietSeurat(mye_sub, assays = "RNA")
```


# 2. Integrate the DFU skin data with mye_sub data
```{r eval=TRUE}
# DFU skin integration using the donor as the batch effect, 
# while the mye_sub data using each sample as the batch effect
DefaultAssay(DFU_skin) = "RNA"
DFU_skin <- DietSeurat(DFU_skin, assays = "RNA")

# check the overlapped genes and keep the overlapped genes
overgene <- intersect(rownames(DFU_skin), rownames(mye_sub))
length(overgene)

DFU_skin <- DFU_skin[overgene, ]
mye_sub <- mye_sub[overgene, ]

# integrate based on sample names. For mye_sub data, use sample ID, 
...
mye_sub$Project="Woundhealing"
mye_sub$Sample=mye_sub$orig.ident
mye_sub$CellType = mye_sub$upCellTypes
```
...
all.mye_sub = SplitObject(mye_sub, split.by = "Sample")
rm(mye_sub);gc()
all.mye_sub <- lapply(X = all.mye_sub, FUN = function(x) {
    x <- NormalizeData(x)
    x <- CellCycleScoring(x, s.features = cc.genes.updated.2019$s.genes, g2m.features = cc.genes.updated.2019$g2m.genes, set.ident = F)
})
for (i in seq_along(all.mye_sub)) {
  all.mye_sub[[i]]$CC.Difference <- all.mye_sub[[i]]$S.Score - all.mye_sub[[i]]$G2M.Score
}

alldata = append(all.DFU, all.mye_sub)
rm(all.DFU, all.mye_sub)
gc(verbose = F)
...
#saveRDS(inteData, "step7_DFU_Wound_Myeloid.rds")
...
data.table::fwrite(cluster_markers_anno2, "step7_DFU_Wound_Myeloid_mg0.5.txt", sep="\t")
...
data.table::fwrite(cluster_markers_anno1, "step7_DFU_Wound_Myeloid_mg0.8.txt", sep="\t")
```
...
```{r eval=TRUE}
rm(list = ls())
inteData <- readRDS("step7_DFU_Wound_Myeloid.rds")
DefaultAssay(inteData) <- "RNA"
inteData <- NormalizeData(inteData)
mg05 <- data.table::fread("step7_DFU_Wound_Myeloid_mg0.5.txt")
mg08 <- data.table::fread("step7_DFU_Wound_Myeloid_mg0.8.txt")
mg08$cluster <- factor(mg08$cluster)
table(inteData$SCT_snn_res.0.8, inteData$Condition)

DimPlot(inteData, group.by = "CellType", split.by = "Project", label = T)
FeaturePlot(inteData, features = c("MMP9", "DAB2", "VEGFA"), cols = c("grey90", "red"), split.by = "Condition") 
FeaturePlot(inteData, features = c("APOE", "IL1B", "MKI67"), cols = c("grey90", "red")) 
...
pdf("step7_DFU_Wound_Myeloid_cellproportion.pdf", useDingbats = F, width = 3.5, height = 4)
p_cp_5
dev.off()
...
step2_sig <- step2 %>% 
  ...
  dplyr::filter(CellType %in% c("Mac_inf", "Mac1")) %>% # choose the cell type
...
```{r}
###################################################
# Fig 5 Calculate the scores of macrophage clusters
# read the marker genes from scRNA-seq data
markers <- data.table::fread("/Users/zhuliu/Desktop/sc st/plotting scripts/Figure 6/DEmarkers_immu_myeloid_mg_res0.5.txt") %>% 
...
macinfgene_sig <- list(FB1= unique(c(markers %>% dplyr::filter(CellTypes == "Mac_inf") %>%
                                  top_n(50, avg_log2FC) %>% pull(gene))))
m1gene_sig <- list(FB2= unique(c(markers %>% dplyr::filter(CellTypes == "Mac1") %>%
                                  top_n(50, avg_log2FC) %>% pull(gene))))
m2gene_sig <- list(FB1= unique(c(markers %>% dplyr::filter(CellTypes == "Mac2") %>%
                                  top_n(50, avg_log2FC) %>% pull(gene))))
m3gene_sig <- list(FB2= unique(c(markers %>% dplyr::filter(CellTypes == "Mac3") %>%
                                  top_n(50, avg_log2FC) %>% pull(gene))))

inteData <- AddModuleScore(object = inteData, features = macinfgene_sig, name = "Mac_inf_Score")
inteData <- AddModuleScore(object = inteData, features = m1gene_sig, name = "Mac1_Score") 

inteData <- AddModuleScore(object = inteData, features = m2gene_sig, name = "Mac2_Score")
inteData <- AddModuleScore(object = inteData, features = m3gene_sig, name = "Mac3_Score") 

VlnPlot(
  inteData,
  features = c("Mac_inf_Score1"),
  ncol = 1, pt.size=0, group.by = "newCellType", split.by = "Condition") +
  VlnPlot(
  inteData,
  features = c("Mac1_Score1"),
  ncol = 1, pt.size=0, group.by = "newCellType", split.by = "Condition")
VlnPlot(
  inteData,
  features = c("Mac2_Score1"),
  ncol = 1, pt.size=0, group.by = "newCellType", split.by = "Condition") +
  VlnPlot(
  inteData,
  features = c("Mac3_Score1"),
  ncol = 1, pt.size=0, group.by = "newCellType", split.by = "Condition")

library(ggpubr)
cluster_score <- inteData@meta.data %>% select(7, 37:41) %>% 
  mutate(Group = gsub("H[0-9]$", "H", Sample)) %>% 
  mutate(Group = gsub("^PWH[0-9]{2}", "", Group)) %>% 
...
sigPlot1 <- ggviolin(cluster_score, x = "newCellType", y = "Mac_inf_Score1", fill = "Group", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons)

sigPlot2 <- ggviolin(cluster_score, x = "newCellType", y = "Mac1_Score1", fill = "Group", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons)
...
(ggviolin(df.prop, x = "Group", y = "Mac_inf_Score1", fill = "Group",palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")) +#, label = "p.signif"
  (ggviolin(df.prop, x = "Group", y = "Mac1_Score1", fill = "Group", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")) #, label = "p.signif"
#pdf("step7_DFU_Wound_Myeloid_Top100mgScores_mac2_mac3_sig.pdf", useDingbats = F, width = 6, height = 3)
#dev.off()
(ggviolin(df.prop, x = "Group", y = "Mac2_Score1", fill = "Group",palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")) +#, label = "p.signif"
  (ggviolin(df.prop, x = "Group", y = "Mac3_Score1", fill = "Group", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
 add = c("mean_sd")) + 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")) #, label = "p.signif"
#pdf("step7_DFU_Wound_Myeloid_Top100mgScores_mac2_mac3_sig.pdf", useDingbats = F, width = 6, height = 3)
#dev.off()
