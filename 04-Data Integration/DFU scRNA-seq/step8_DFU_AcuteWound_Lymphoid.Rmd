...
DFU_skin <- subset(inteData, subset = CellType == "NKT_Lympho" | CellType == "B_Lympho")
...
DimPlot(lym_sub, group.by = "upCellTypes", cols = ct.cols)
lym_sub <- DietSeurat(lym_sub, assays = "RNA")
```


# 2. Integrate the DFU skin data with lym_sub data
```{r eval=TRUE}
# DFU skin integration using the donor as the batch effect, 
# while the lym_sub data using each sample as the batch effect
DefaultAssay(DFU_skin) = "RNA"
DFU_skin <- DietSeurat(DFU_skin, assays = "RNA")

# check the overlapped genes and keep the overlapped genes
overgene <- intersect(rownames(DFU_skin), rownames(lym_sub))
length(overgene)

DFU_skin <- DFU_skin[overgene, ]
lym_sub <- lym_sub[overgene, ]

# integrate based on sample names. For lym_sub data, use sample ID, 
...
lym_sub$Project="Woundhealing"
lym_sub$Sample=lym_sub$orig.ident
lym_sub$CellType = lym_sub$upCellTypes
```
...
all.lym_sub = SplitObject(lym_sub, split.by = "Sample")
rm(lym_sub);gc()
all.lym_sub <- lapply(X = all.lym_sub, FUN = function(x) {
    x <- NormalizeData(x)
    x <- CellCycleScoring(x, s.features = cc.genes.updated.2019$s.genes, g2m.features = cc.genes.updated.2019$g2m.genes, set.ident = F)
})
for (i in seq_along(all.lym_sub)) {
  all.lym_sub[[i]]$CC.Difference <- all.lym_sub[[i]]$S.Score - all.lym_sub[[i]]$G2M.Score
}

alldata = append(all.DFU, all.lym_sub)
rm(all.DFU, all.lym_sub)
gc(verbose = F)
...
#saveRDS(inteData, "step8_DFU_Wound_Lymphoid.rds")
...
data.table::fwrite(cluster_markers_anno2, "step8_DFU_Wound_Lymphoid_mg0.5.txt", sep="\t")
...
data.table::fwrite(cluster_markers_anno1, "step8_DFU_Wound_Lymphoid_mg0.8.txt", sep="\t")
```
...
```{r eval=TRUE}
rm(list = ls())
inteData <- readRDS("step8_DFU_Wound_Lymphoid.rds")
DefaultAssay(inteData) <- "RNA"
inteData <- NormalizeData(inteData)
mg05 <- data.table::fread("step8_DFU_Wound_Lymphoid_mg0.5.txt")
mg08 <- data.table::fread("step8_DFU_Wound_Lymphoid_mg0.8.txt")
...
FeaturePlot(inteData, features = c("CD3D", "KLRB1", "CD4", "CD8A",
                                   "CD3G", "KLRD1", "CD40LG", "CD8B"), cols = c("grey90", "red"), ncol = 4) 
...
pdf("step8_DFU_Wound_Lymphoid_cellproportion.pdf", useDingbats = F, width = 3.5, height = 4)
p_cp_5
dev.off()
...
step2_sig <- step2 %>% 
  ...
  dplyr::filter(CellType %in% c("Bcell")) %>% # choose the cell type
