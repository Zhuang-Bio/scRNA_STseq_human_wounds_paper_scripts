...
hswound.combined.sct <- readRDS("00-tmpRawfiles/s1_ori_immu_lymphoid.rds") 
...
```{r}
table(hswound.combined.sct$SCT_snn_res.1)
hswound.combined.sct$fill <- ifelse(hswound.combined.sct$SCT_snn_res.1 %in% c(16, 17), "Yes", "No")
...
#hswound.immu.com <- RunUMAP(object = hswound.immu.com, assay = "SCT", reduction = "harmony", dims = #1:20, n.neighbors = 40, min.dist = 0.3, n.epochs = 500, spread = 1, learning.rate = 1)
...
data.table::fwrite(markers_f, file="s1_clean_immu_lymphoid_mg_res0.8.txt", sep="\t") 

hswound.immu.com <- NormalizeData(hswound.immu.com, verbose = TRUE,  normalization.method = "LogNormalize", scale.factor = 10000)

saveRDS(hswound.immu.com, file = "s1_clean_immu_lymphoid.rds")
```
...
plotcom <- FeaturePlot(
  hswound.immu.com,
  features=c('CD8A', 'CD8B', 'CD4', 'CD40LG', 'FOXP3', 'TIGIT', 'CTLA4'), 
