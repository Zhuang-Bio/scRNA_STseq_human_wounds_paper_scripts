...
seu_obj <- readRDS(paste0(output_path, "s1_clean_fibroblast.rds"))
...
```{r}
# build KNN graph
seu_milo <- buildGraph(seu_milo, k = 40, d = 30, reduced.dim = "HARMONY")

# Defining representative neighbourhoods on the KNN graph
seu_milo <- makeNhoods(seu_milo, prop = 0.1, k = 40, d = 30, refined = TRUE, reduced_dims = "HARMONY", refinement_scheme="reduced_dim")
...
pdf("Milo_FB_wound1_skin.pdf", useDingbats = FALSE, width = 12, height = 6)
dev.off()
...
# Draw the plot
da_results$nh_graph_pl + 
  (da_results$beeplot + scale_y_continuous(breaks = c(-4,-2,0,2,4,6,8), limits = c(-4,8))) + 
  plot_layout(widths = c(1.6, 1.2))
pdf("Milo_FB_wound7_wound1.pdf", useDingbats = FALSE, width = 12, height = 6)
dev.off()
...
# Draw the plot
da_results$nh_graph_pl + 
  (da_results$beeplot + scale_y_continuous(breaks = c(-8,-6,-4,-2,0,2,4,6), limits = c(-8,6))) + 
  plot_layout(widths = c(1.6, 1.2))
pdf("Milo_FB_wound30_wound7.pdf", useDingbats = FALSE, width = 12, height = 6)
dev.off()
```
...
pdf("Fig_cellabundance_analysis_FB.pdf", useDingbats = FALSE, width = 12, height = 6)
da_results$nh_graph_pl + 
  (da_results$beeplot + scale_y_continuous(breaks = c(-4,-2,0,2,4,6), limits = c(-4,7)))
dev.off()
```
