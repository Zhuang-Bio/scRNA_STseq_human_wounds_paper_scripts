...
# 12. Human lncRNA and protein-coding genes expression
```{r}
...
# Migration and proliferation scores
```{r}
conserved_markers_anno <- data.table::fread("Manuscript_newCluster_markergenes.txt")
conserved_markers_anno$cluster <- as.factor(conserved_markers_anno$cluster)
...
library(tidyverse)
# markers
markers <- data.table::fread("Manuscript_newCluster_markergenes.txt")
top200markers <- markers %>% filter(Wound1_p_val_adj<0.001 & DPW3_p_val_adj<0.001)

data.table::fwrite(top200markers, "Markers_top200_crossspecies_conserved.txt", sep = "\t")
